...
import { BaseCSS } from './lit-wired-base';
...
export class LitWiredTabs extends LitElement {
        ...
        ${this.pages.map(
          (p) => html`
            <wired-item
              role="tab"
              .value="${p.name}"
              .selected="${p.name === this.selected}"
              ?aria-selected="${p.name === this.selected}"
              @click="${() => (this.selected = p.name)}"
              >${p.label || p.name}</wired-item
            >
          `,
        )}
      </div>
      ...
      if (assigned && assigned.length) {
        for (let i = 0; i < assigned.length; i++) {
          ...
          if (
            ...
            (n as HTMLElement).tagName.toLowerCase() === 'wired-tab'
          ) {
